...
function update() {
    ...
    if (!gameState.boss && gameState.enemies.length === 0 && gameState.kills > 0) {        if (Math.random() < 0.002) showWaveComplete();
    }
}
...
function startGame() {
    ...
    document.getElementById('weaponStatus').classList.add('visible');
...
function gameOver() {
    ...
    if (isNewBest) {
        ...
        bestIndicator.style.cssText = `
            position: absolute;
            top: 20px;
            left: 50%;
            transform: translateX(-50%);
            background: linear-gradient(135deg, #ffea00, #ff00e4);
            color: #050508;
            padding: 1rem 2rem;
            border-radius: 50px;
    ...
    document.getElementById('weaponStatus').classList.remove('visible');
}
...
function initCustomCursor() {
    ...
    const hoverElements = document.querySelectorAll('a, button, .cta-btn, .launch-btn, .diff-option');
